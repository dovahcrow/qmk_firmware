...
static void settle_as_tap(void) {
  if (eager_mods) {  // Clear eager mods if set.
    ...
    // tap_hold_record.tap.interrupted = false;
  ...
  dprintf("Achordion: Plumbing tap press for %#02X.\n", tap_hold_keycode);
  ...
  dprintf("Achordion: Plumbing tap release for %#02X.\n", tap_hold_keycode);
  tap_hold_record.event.pressed = false;
  ...
  if (achordion_state == STATE_RELEASED) {
    if (is_tap_hold && record->tap.count == 0 && record->event.pressed &&
      ...
      if (timeout > 0) {
        ...
        dprintf("Achordion: Saving tap press for %#02X.\n", keycode);
